...
class SuperHero extends SuperHuman {
	...
	battle(SV){
		if (this.powerLevel >= SV.powerLevel){
			return true;
		} else {
			return false;
		}
...
function selectionChanged() {
	...
	// Your code goes here
	let re = selectedHero.battle(selectedVillain);
	let sl = document.getElementById("winner");
	
	if (re === false){
		sl.innerHTML = '<p>' +  'Winner: ' + selectedVillain.alias + '</p>';
	} else {
		sl.innerHTML = '<p>' + 'Winner: ' +  selectedHero.alias + '</p>';
	}
